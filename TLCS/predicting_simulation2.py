...
class Simulation:
    def __init__(self, Model, max_steps, green_duration, yellow_duration, num_states, num_actions, lanes):
        ...
        self._green_duration = green_duration-1
        self._yellow_duration = yellow_duration-1
    ...
    def run(self, car_queue):
        ...
        if self._step < self._max_steps:
            ...
            # if the chosen phase is different from the last phase, activate the yellow phase
                if self._step != 0 and self._old_action != self._action:
                    result = self._set_yellow_phase(self._old_action)
            ...
            if self._isyellow:
                self._isyellow = False
            result = self._set_green_phase(self._action)
        ...
        return result #0 #simulation_time
    ...
    def _get_state(self,old_action, car_queue):
        ...
        if old_action==0:
            flow_count = (lane_car[2] + lane_car[6])/self._lanes
            lane_car[2]=0; lane_car[6] = 0
            state[8] = flow_count
            total_flow += flow_count
        if old_action==1:
            flow_count = (lane_car[3] + lane_car[7])/self._lanes
            lane_car[3]=0; lane_car[7]=0
            state[9] = flow_count
            total_flow += flow_count
        if old_action==2:
            flow_count = (lane_car[0] + lane_car[4])/self._lanes
            lane_car[0]=0; lane_car[4]=0
            state[10] = flow_count
            total_flow += flow_count
        if old_action==3:
            flow_count = (lane_car[1] + lane_car[5])/self._lanes
        ...
        for i in range(8):
            total_s = 0
            if i%2==0:
                for j in np.arange(0, lane_car[i]/self._lanes, 0.1):
